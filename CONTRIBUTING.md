...
We want this community to be friendly and respectful to each other. Please follow it in all your
interactions with the project. Before contributing, please read
...
Objective-C or Swift files, open `example/ios/ReactNativeShortcutsExample.xcworkspace` in XCode and
find the source files at `Pods > Development Pods > @rn-bridge/react-native-shortcuts`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files
at `rn-bridge-react-native-shortcuts` under `Android`.
...
We use [release-it](https://github.com/release-it/release-it) to make it easier to publish new
versions. It handles common tasks like bumping version based on semver, creating tags and releases
etc.
